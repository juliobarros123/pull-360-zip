            ...
            <div class="flex justify-between flex-1 sm:hidden">
                ...
                <span>
                    @if ($paginator->hasMorePages())
                        <button wire:click="PróximoPage" wire:loading.attr="disabled" dusk="PróximoPage.before" class="relative inline-flex items-center px-4 py-2 ml-3 text-sm font-medium text-gray-700 bg-white border border-gray-300 leading-5 rounded-md hover:text-gray-500 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150">
                            {!! __('pagination.Próximo') !!}
                        </button>
                    @else
                ...
                <div>
                    <span class="relative z-0 inline-flex shadow-sm">
                        ...
                        <span>
                            {{-- Próximo Page Link --}}
                            @if ($paginator->hasMorePages())
                                <button wire:click="PróximoPage" dusk="PróximoPage.after" rel="Próximo" class="relative inline-flex items-center px-2 py-2 -ml-px text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-r-md leading-5 hover:text-gray-400 focus:z-10 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:bg-gray-100 active:text-gray-500 transition ease-in-out duration-150" aria-label="{{ __('pagination.Próximo') }}">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" />
                                    </svg>
                                </button>
                            @else
                                <span aria-disabled="true" aria-label="{{ __('pagination.Próximo') }}">

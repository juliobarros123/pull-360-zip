...
<h3>
    <div class="text-center">RELATÓRIO DO NÚMERO DE ALUNOS POR PROVÍNCIA  {{ isset($anoLectivo)?'NO ANO LECTIVO '.$anoLectivo->ya_inicio.'/'.$anoLectivo->ya_fim:'' }}</div>
</h3>
        ...
        @foreach ($provincias as $provincia)
            <tr class="text-center">
                ...
                <td class="text-center">
                    {{ $dados->where('it_id_provincia', $provincia)->where('vc_genero', 'Masculino')->sum('quantidade') }}
                </td>
                <td class="text-center">{{ $dados->where('it_id_provincia', $provincia)->where('vc_genero', 'Feminino')->count() }}</td>
            </tr>
        @endforeach
        <tr class="text-center">

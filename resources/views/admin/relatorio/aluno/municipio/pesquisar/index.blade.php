...
@section('titulo', ' Relatório de alunos por município')
    ...
    <div class="page-wrapper p-2 p-md-5">
        <div class="p-3">
            ...
            <div class="w-100 bg-white shadow p-4 h-100 mt-3 rounded">
                ...
                <form action="{{ route('relatorio.aluno.municipio.imprimirAlunosPorMunicipio') }}" method="post">
                    @csrf
                    <div class="row">


                        <div class="form-group col-sm-6">
                            <label class="bold" for="id_municipio">{{ __('Selecciona o município*') }}</label>
                            <select class="form-control bg-white rounded-pill select2" name="id_municipio" id="id_municipio">
                        
                        
                        
                                <option  value="Todos municípios" selected>Todos municípios</option>
                        
                        
                                @foreach ($municipios as $municipio)
                                    <option value="{{ $municipio->it_id_minicipio }}">{{ $municipio->it_id_minicipio }}</option>
                                @endforeach
